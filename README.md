...
**已知限制**：同一个进程内并发调用 `addItems`（比如以后做了网页/webhook 入口，同一个服务进程同时处理多个客户提交）已经做了排队，不会互相覆盖。但如果是**两个独立进程同时**跑导入命令（比如两台机器、或两次手动同时执行命令行），仍然可能互相覆盖对方的写入——JSON 文件本身不支持跨进程加锁。单人手动一次跑一个导入没问题；真要支持多进程并发提交，接入真数据库（有事务）之前不要让它承担这种场景。
